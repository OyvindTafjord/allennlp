...
from ernie.bert_tokenizer_and_candidate_generator import TokenizerAndCandidateGenerator

logger = logging.getLogger(__name__)  # pylint: disable=invalid-name


@DatasetReader.register("kermit_mc_qa")
class KermitMCQAReader(DatasetReader):
    """
    ...
    def __init__(self,
        ...
        self._word_splitter = BertBasicWordSplitter(do_lower_case=False)
    ...
    def text_to_instance(self,  # type: ignore
        ...
        for idx, choice in enumerate(choice_list):
            ...
            text_a, text_b = self._normalized_text_from_qa(question, choice, choice_context)
            token_candidates = self.tokenizer.tokenize_and_generate_candidates(text_a, text_b)
            fields_sub = self.tokenizer.convert_tokens_candidates_to_fields(token_candidates)
            qa_fields.append(fields_sub['tokens'])
            segment_ids_fields.append(fields_sub['segment_ids'])
            candidates_fields.append(fields_sub['candidates'])
            qa_tokens = fields_sub['tokens'].tokens
            qa_tokens_list.append(qa_tokens)
            if debug > 0:
                logger.info(f"qa_tokens = {qa_tokens}")
                logger.info(f"segment_ids = {fields_sub['segment_ids'].array}")

        fields['question'] = ListField(qa_fields)
        fields['segment_ids'] = ListField(segment_ids_fields)
        fields['candidates'] = ListField(candidates_fields)
    ...
    def _normalized_text_from_qa(self, question: str, answer: str, context: str = None):
        sep_str = "[SEP]"
        prefix = ""
        if context is not None:
            prefix_tokens = self._word_splitter.split_words(context)
            prefix = " ".join(t.text for t in prefix_tokens) + " " + sep_str + " "
        question_str = " ".join(t.text for t in self._word_splitter.split_words(question))
        answer_str = " ".join(t.text for t in self._word_splitter.split_words(answer))
        text_a = prefix + question_str
        text_b = answer_str
